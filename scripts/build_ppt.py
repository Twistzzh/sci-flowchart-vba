    ...
    for fn in files:
        ...
        for line in logical.splitlines():
            ...
            if line.startswith("AddNode "):
                ...
                formulas.append(parse_formula(split_args(line[11:])))
    ...
    for line in decor:
        if line.startswith("SetPara "):
            ...
            for n in nodes:
                if n["id"] == nid:
                    n["prefix"] = (nc, ctoken)
    return pal, scal, nodes, paths, formulas
...
def latex_to_omml(tex: str, pt: float) -> str:
    ...
    om = re.sub(
        ...
        run_fix, om)
    # runs without a style entry still need the size
...
def build_with_replay(out_dir: str, out_path: str) -> int:
    ...
    pal, scal, nodes, paths, formulas = load_bas(out_dir)
    ...
    for f in formulas:
        ...
        for para in tf.paragraphs:
            ...
            for r in para.runs:
                ...
                r.font.italic = True
                r.font.name = g["font"]
                fc = resolve_color(f["ctok"], pal) or (31, 31, 31)
                r.font.color.rgb = RGBColor(*fc)

    prs.save(out_path)
    rc = inject_math(out_path, formulas)
    return 0
...
def main() -> int:
    ...
    used = None
    _, _, _, _, formulas = load_bas(out_dir)
